...
def parse_option():
    ...
    parser.add_argument('--min_delta', default=1e-5)
    ...
    parser.add_argument('--input_dir', default='data/PI_train/Teacher32_batch64_FCinput_PCD.npy')
    parser.add_argument('--output_dir', default='data/PI_train/Teacher32_batch64_FCinput_PI_dim0.npy')
...
def main():
    ...
    opt.ckpt_dir = 'rips_%s_ckpt/trial_%s' %(opt.operator, opt.rips_trial)
    opt.log_dir = 'rips_%s_logs/trial_%s' %(opt.operator, opt.rips_trial)    
    ...
    for epoch in range(opt.epochs):
        ...
        with torch.no_grad():
            ...
            if val_cost < val_cost_:
                state = {'epoch' : epoch,
                        'model' : model.state_dict(),
                        'best_acc' : val_cost,
                        'optimizer' : optimizer.state_dict()}
                save_file = os.path.join(opt.ckpt_dir, '{}_dim{}_best.pth'.format(opt.input_dir.split('/')[-1][:-8], opt.dim))
                logger.info('saving the best model!')
                torch.save(state, save_file)

                if abs(val_cost_ - val_cost) < opt.min_delta:
                    break
                else:
                    val_cost_ = val_cost
